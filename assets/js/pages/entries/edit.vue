...
<template>
  <Head :title="`Edit: ${entry.title} | FieldNotes`" />
  ...
  <section class="mx-auto max-w-2xl px-4 pt-10">
    ...
    <div>
      <div class="mb-8 flex items-center justify-between">
        <h1 class="text-xl font-bold tracking-tight text-black">Edit Entry</h1>
        <span
          class="inline-flex rounded-lg bg-gray-100/80 px-2.5 py-1 text-xs font-medium capitalize text-gray-500"
        >
          {{ entry.type }}
        </span>
      </div>
      ...
      <form @submit.prevent="submit" class="space-y-6">
        ...
        <div v-if="entry.type === 'task'" class="grid grid-cols-2 gap-4">
          <div>
